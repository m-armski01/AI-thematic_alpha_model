...
| `universe.selection` | `owned_portfolio` (was `hindsight` until 2026-09-17) | `point_in_time` (ETF config only) | Report framing, not a strategy parameter: `owned_portfolio` states that the names are the author's holdings and frames the report as overlay vs equal-weight buy-and-hold of the same basket; `point_in_time` states how the control universe was defined and the residual ETF-delisting bias. |
...
- 2026-09-17 — **Report reframing.** `universe.selection: hindsight` is renamed `owned_portfolio` (still report framing only); a `report:` block (title, subtitle, author, thesis, disclaimer) is added with defaults that reproduce the previous header. Reports now lead with the thesis, a framing paragraph and a verdict generated from the numbers against equal-weight buy-and-hold of the basket; SPY and naive momentum move to a market-context appendix; alpha is reported with its beta and a t-statistic; CAGR, Calmar and annualised alpha are suppressed on windows under 24 months (504 sessions). No target, trade or metric value changes. Reason: IMPROVEMENTS.md §4–6.
